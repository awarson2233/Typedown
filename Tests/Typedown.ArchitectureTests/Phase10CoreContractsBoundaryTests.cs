...
public class Phase10CoreContractsBoundaryTests
{
    ...
    public void WinUIPackagedBaseline_UsesSolutionDebugX64BuildAndDeployWithoutLegacyProjects()
    {
        ...
        AssertHasTypeReference(solutionSource, $"{editorGuid}.Debug_Local|x64.Build.0 = Debug_Local|Any CPU");
        AssertHasTypeReference(solutionSource, $"{editorGuid}.Debug|ARM64.Build.0 = Debug|Any CPU");
        AssertHasTypeReference(solutionSource, $"{editorGuid}.Release|ARM64.Build.0 = Release|Any CPU");
    ...
    {
        var projectSource = File.ReadAllText(Path.Combine(RepoRoot, "Dev", "Typedown.WinUI", "Typedown.WinUI.csproj"));
        var editorProjectSource = File.ReadAllText(Path.Combine(RepoRoot, "Dev", "Typedown.Editor", "Typedown.Editor.esproj"));
        ...
        AssertHasTypeReference(projectSource, @"<ProjectReference Include=""..\Typedown.Editor\Typedown.Editor.esproj""");
        AssertHasTypeReference(projectSource, "ReferenceOutputAssembly=\"false\"");
        AssertHasTypeReference(projectSource, "SkipGetTargetFrameworkProperties=\"true\"");
        AssertHasTypeReference(projectSource, "SetPlatform=\"Platform=AnyCPU\"");
        AssertHasTypeReference(projectSource, "Target Name=\"RefreshTypedownEditorStaticBundleContent\"");
        AssertHasTypeReference(projectSource, "DependsOnTargets=\"ResolveProjectReferences\"");
        AssertHasTypeReference(projectSource, "AfterTargets=\"ResolveProjectReferences\"");
        AssertHasTypeReference(projectSource, "BeforeTargets=\"AssignTargetPaths;GetCopyToOutputDirectoryItems;GetPackagingOutputs\"");
        AssertHasTypeReference(projectSource, "Typedown.Editor static bundle was not generated by its ProjectReference.");
        ...
        AssertNoTypeReference(projectSource, "Target Name=\"BuildTypedownEditorStaticBundle\"");
        AssertHasTypeReference(editorProjectSource, "<BuildCommand>yarn build</BuildCommand>");
        AssertHasTypeReference(editorProjectSource, "<BuildWorkingDirectory>$(MSBuildProjectDirectory)</BuildWorkingDirectory>");
        AssertHasTypeReference(editorProjectSource, @"<BuildOutputFolder>$(MSBuildProjectDirectory)\..\Typedown.WinUI\Resources\Statics</BuildOutputFolder>");
        AssertHasTypeReference(editorProjectSource, "<ProductionBuildCommand></ProductionBuildCommand>");
        AssertHasTypeReference(editorProjectSource, "<Import Project=\"Sdk.props\" Sdk=\"Microsoft.VisualStudio.JavaScript.Sdk\" Version=\"0.5.45-alpha\" />");
        AssertHasTypeReference(editorProjectSource, "<Import Project=\"Sdk.targets\" Sdk=\"Microsoft.VisualStudio.JavaScript.Sdk\" Version=\"0.5.45-alpha\" />");
        AssertHasTypeReference(editorProjectSource, "Target Name=\"EnsureYarnNodeEnv\"");
        AssertHasTypeReference(editorProjectSource, "BeforeTargets=\"CoreCompile\"");
        AssertHasTypeReference(editorProjectSource, "Command=\"yarn install --frozen-lockfile\"");
        AssertHasTypeReference(editorProjectSource, "Target Name=\"DebugEnsureNodeEnv\" DependsOnTargets=\"EnsureYarnNodeEnv\"");
        AssertNoTypeReference(editorProjectSource, "ShouldRunNpmInstall");
        AssertNoTypeReference(editorProjectSource, "Command=\"npm install\"");
        AssertHasTypeReference(projectSource, "Target Name=\"CleanTypedownEditorStaticBundleOutput\"");
        AssertHasTypeReference(projectSource, "BeforeTargets=\"_CopyOutOfDateSourceItemsToOutputDirectoryAlways\"");
        AssertHasTypeReference(projectSource, "Condition=\"'$(OutDir)' != '' and Exists('$(OutDir)$(TypedownEditorStaticBundleStagingPath)')\"");
        AssertHasTypeReference(projectSource, "<RemoveDir Directories=\"$(OutDir)$(TypedownEditorStaticBundleStagingPath)\" />");
    ...
    public void AppAndActivationService_KeepStartupActivationPipelineAndCommandLineOpenNewWindowFlow()
    {
        ...
        AssertContainsInOrder(
            ...
            "catch (Exception ex)",
            "Debug.WriteLine($\"Typedown activation redirection cancellation request failed",
