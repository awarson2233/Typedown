...
import { n, t as r } from "./src-upoOno_g.mjs";
import { A as i, F as a, b as o, s, z as c } from "./chunk-WYO6CB5R-_U4dPty4.mjs";
import { a as l } from "./chunk-ICXQ74PX-5xZ0PKaW.mjs";
import { n as u, t as d } from "./chunk-HOUHSVGY-2PLbOYM-.mjs";
...
var ee = (() => {
...
}, te = /^(?:[ \t]*(?:\n|$))+/, ne = /^((?: {4}| {0,3}\t)[^\n]+(?:\n(?:[ \t]*(?:\n|$))*)?)+/, re = /^ {0,3}(`{3,}(?=[^`\n]*(?:\n|$))|~{3,})([^\n]*)(?:\n|$)(?:|([\s\S]*?)(?:\n|$))(?: {0,3}\1[~`]* *(?=\n|$)|$)/, y = /^ {0,3}((?:-[\t ]*){3,}|(?:_[ \t]*){3,}|(?:\*[ \t]*){3,})(?:\n+|$)/, ie = /^ {0,3}(#{1,6})(?=\s|$)(.*)(?:\n+|$)/, b = /(?:[*+-]|\d{1,9}[.)])/, ae = /^(?!bull |blockCode|fences|blockquote|heading|html|table)((?:.|\n(?!\s*?\n|bull |blockCode|fences|blockquote|heading|html|table))+?)\n {0,3}(=+|-+) *(?:\n+|$)/, oe = _(ae).replace(/bull/g, b).replace(/blockCode/g, /(?: {4}| {0,3}\t)/).replace(/fences/g, / {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g, / {0,3}>/).replace(/heading/g, / {0,3}#{1,6}/).replace(/html/g, / {0,3}<[^\n>]+>\n/).replace(/\|table/g, "").getRegex(), se = _(ae).replace(/bull/g, b).replace(/blockCode/g, /(?: {4}| {0,3}\t)/).replace(/fences/g, / {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g, / {0,3}>/).replace(/heading/g, / {0,3}#{1,6}/).replace(/html/g, / {0,3}<[^\n>]+>\n/).replace(/table/g, / {0,3}\|?(?:[:\- ]*\|)+[\:\- ]*\n/).getRegex(), x = /^([^\n]+(?:\n(?!hr|heading|lheading|blockquote|fences|list|html|table| +\n)[^\n]+)*)/, ce = /^[^\n]+/, S = /(?!\s*\])(?:\\[\s\S]|[^\[\]\\])+/, le = _(/^ {0,3}\[(label)\]: *(?:\n[ \t]*)?([^<\s][^\s]*|<.*?>)(?:(?: +(?:\n[ \t]*)?| *\n[ \t]*)(title))? *(?:\n+|$)/).replace("label", S).replace("title", /(?:"(?:\\"?|[^"\\])*"|'[^'\n]*(?:\n[^'\n]+)*\n?'|\([^()]*\))/).getRegex(), ue = _(/^( {0,3}bull)([ \t][^\n]+?)?(?:\n|$)/).replace(/bull/g, b).getRegex(), C = "address|article|aside|base|basefont|blockquote|body|caption|center|col|colgroup|dd|details|dialog|dir|div|dl|dt|fieldset|figcaption|figure|footer|form|frame|frameset|h[1-6]|head|header|hr|html|iframe|legend|li|link|main|menu|menuitem|meta|nav|noframes|ol|optgroup|option|p|param|search|section|summary|table|tbody|td|tfoot|th|thead|title|tr|track|ul", w = /<!--(?:-?>|[\s\S]*?(?:-->|$))/, de = _("^ {0,3}(?:<(script|pre|style|textarea)[\\s>][\\s\\S]*?(?:</\\1>[^\\n]*\\n+|$)|comment[^\\n]*(\\n+|$)|<\\?[\\s\\S]*?(?:\\?>\\n*|$)|<![A-Z][\\s\\S]*?(?:>\\n*|$)|<!\\[CDATA\\[[\\s\\S]*?(?:\\]\\]>\\n*|$)|</?(tag)(?: +|\\n|/?>)[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|<(?!script|pre|style|textarea)([a-z][\\w-]*)(?:attribute)*? */?>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|</(?!script|pre|style|textarea)[a-z][\\w-]*\\s*>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$))", "i").replace("comment", w).replace("tag", C).replace("attribute", / +[a-zA-Z:_][\w.:-]*(?: *= *"[^"\n]*"| *= *'[^'\n]*'| *= *[^\s"'=<>`]+)?/).getRegex(), fe = _(x).replace("hr", y).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("|table", "").replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)]) ").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", C).getRegex(), T = {
	...
	table: g,
	text: ce
}, pe = _("^ *([^\\n ].*)\\n {0,3}((?:\\| *)?:?-+:? *(?:\\| *:?-+:? *)*(?:\\| *)?)(?:\\n((?:(?! *\\n|hr|heading|blockquote|code|fences|list|html).*(?:\\n|$))*)\\n*|$)").replace("hr", y).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("blockquote", " {0,3}>").replace("code", "(?: {4}| {0,3}	)[^\\n]").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)]) ").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", C).getRegex(), me = {
	...T,
	lheading: se,
	table: pe,
	paragraph: _(x).replace("hr", y).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("table", pe).replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)]) ").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", C).getRegex()
}, he = {
	...T,
...
}, ge = /^\\([!"#$%&'()*+,\-./:;<=>?@\[\]\\^_`{|}~])/, _e = /^(`+)([^`]|[^`][\s\S]*?[^`])\1(?!`)/, ve = /^( {2,}|\\)\n(?!\s*$)/, ye = /^(`+|[^`])(?:(?= {2,}\n)|[\s\S]*?(?:(?=[\\<!\[`*_]|\b_|$)|[^ ](?= {2,}\n)))/, E = /[\p{P}\p{S}]/u, D = /[\s\p{P}\p{S}]/u, be = /[^\s\p{P}\p{S}]/u, xe = _(/^((?![*_])punctSpace)/, "u").replace(/punctSpace/g, D).getRegex(), Se = /(?!~)[\p{P}\p{S}]/u, Ce = /(?!~)[\s\p{P}\p{S}]/u, we = /(?:[^\s\p{P}\p{S}]|~)/u, Te = _(/link|precode-code|html/, "g").replace("link", /\[(?:[^\[\]`]|(?<a>`+)[^`]+\k<a>(?!`))*?\]\((?:\\[\s\S]|[^\\\(\)]|\((?:\\[\s\S]|[^\\\(\)])*\))*\)/).replace("precode-", ee ? "(?<!`)()" : "(^^|[^`])").replace("code", /(?<b>`+)[^`]+\k<b>(?!`)/).replace("html", /<(?! )[^<>]*?>/).getRegex(), O = /^(?:\*+(?:((?!\*)punct)|[^\s*]))|^_+(?:((?!_)punct)|([^\s_]))/, Ee = _(O, "u").replace(/punct/g, E).getRegex(), De = _(O, "u").replace(/punct/g, Se).getRegex(), k = "^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)", Oe = _(k, "gu").replace(/notPunctSpace/g, be).replace(/punctSpace/g, D).replace(/punct/g, E).getRegex(), ke = _(k, "gu").replace(/notPunctSpace/g, we).replace(/punctSpace/g, Ce).replace(/punct/g, Se).getRegex(), Ae = _("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)punct(_+)(?=[\\s]|$)|notPunctSpace(_+)(?!_)(?=punctSpace|$)|(?!_)punctSpace(_+)(?=notPunctSpace)|[\\s](_+)(?!_)(?=punct)|(?!_)punct(_+)(?!_)(?=punct)", "gu").replace(/notPunctSpace/g, be).replace(/punctSpace/g, D).replace(/punct/g, E).getRegex(), je = _(/\\(punct)/, "gu").replace(/punct/g, E).getRegex(), Me = _(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/).replace("scheme", /[a-zA-Z][a-zA-Z0-9+.-]{1,31}/).replace("email", /[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/).getRegex(), Ne = _(w).replace("(?:-->|$)", "-->").getRegex(), Pe = _("^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>").replace("comment", Ne).replace("attribute", /\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/).getRegex(), A = /(?:\[(?:\\[\s\S]|[^\[\]\\])*\]|\\[\s\S]|`+[^`]*?`+(?!`)|[^\[\]\\`])*?/, Fe = _(/^!?\[(label)\]\(\s*(href)(?:(?:[ \t]*(?:\n[ \t]*)?)(title))?\s*\)/).replace("label", A).replace("href", /<(?:\\.|[^\n<>\\])+>|[^ \t\n\x00-\x1f]*/).replace("title", /"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/).getRegex(), j = _(/^!?\[(label)\]\[(ref)\]/).replace("label", A).replace("ref", S).getRegex(), Ie = _(/^!?\[(ref)\](?:\[\])?/).replace("ref", S).getRegex(), Le = _("reflink|nolink(?!\\()", "g").replace("reflink", j).replace("nolink", Ie).getRegex(), Re = /[hH][tT][tT][pP][sS]?|[fF][tT][pP]/, M = {
	_backpedal: g,
	anyPunctuation: je,
	autolink: Me,
	blockSkip: Te,
	...
	...M,
	link: _(/^!?\[(label)\]\((.*?)\)/).replace("label", A).getRegex(),
	reflink: _(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label", A).getRegex()
}, N = {
	...M,
	emStrongRDelimAst: ke,
	emStrongLDelim: De,
	url: _(/^((?:protocol):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/).replace("protocol", Re).replace("email", /[A-Za-z0-9._+-]+(@)[a-zA-Z0-9-_]+(?:\.[a-zA-Z0-9-_]*[a-zA-Z0-9])+(?![-_])/).getRegex(),
	_backpedal: /(?:[^?!.,:;*_'"~()&]+|\([^)]*\)|&(?![a-zA-Z0-9]+;$)|[?!.,:;*_'"~)]+(?!$))+/,
	del: /^(~~?)(?=[^\s~])((?:\\[\s\S]|[^\\])*?(?:\\[\s\S]|[^\s~\\]))\1(?=[^~]|$)/,
	text: _(/^([`~]+|[^`~])(?:(?= {2,}\n)|(?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)|[\s\S]*?(?:(?=[\\<!\[`*~_]|\b_|protocol:\/\/|www\.|$)|[^ ](?= {2,}\n)|[^a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-](?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)))/).replace("protocol", Re).getRegex()
}, Be = {
	...N,
	br: _(ve).replace("{2,}", "*").getRegex(),
	text: _(N.text).replace("\\b_", "\\b_| {2,}\\n").replace(/\{2,\}/g, "*").getRegex()
}, P = {
	normal: T,
	gfm: me,
	pedantic: he
...
function L(e, t) {
	if (t) {
		if (v.escapeTest.test(e)) return e.replace(v.escapeReplace, I);
	} else if (v.escapeTestNoEncode.test(e)) return e.replace(v.escapeReplaceNoEncode, I);
	return e;
}
function He(e) {
	try {
...
function Ue(e, t) {
	var n;
	let r = e.replace(v.findPipe, (e, t, n) => {
		let r = !1, i = t;
		for (; --i >= 0 && n[i] === "\\";) r = !r;
		return r ? "|" : " |";
	}).split(v.splitPipe), i = 0;
	if (r[0].trim() || r.shift(), r.length > 0 && !((n = r.at(-1)) != null && n.trim()) && r.pop(), t) if (r.length > t) r.splice(t);
	else for (; r.length < t;) r.push("");
	for (; i < r.length; i++) r[i] = r[i].trim().replace(v.slashPipe, "|");
	return r;
}
function R(e, t, n) {
	let r = e.length;
...
var z = class {
	constructor(t) {
		e(this, "options", void 0), e(this, "rules", void 0), e(this, "lexer", void 0), this.options = t || m;
	}
	space(e) {
...
${c}` : c;
	...
	table(e) {
		...
		let r = Ue(n[1]), i = n[2].replace(this.rules.other.tableAlignChars, "").split("|"), a = (t = n[3]) != null && t.trim() ? n[3].replace(this.rules.other.tableRowBlankLine, "").split("\n") : [], o = {
			type: "table",
			raw: n[0],
			header: [],
			align: [],
		...
		if (r.length === i.length) {
			...
			for (let e of a) o.rows.push(Ue(e, o.header.length).map((e, t) => ({
				text: e,
				tokens: this.lexer.inline(e),
				header: !1,
				align: o.align[t]
	...
	constructor(t) {
		e(this, "tokens", void 0), e(this, "options", void 0), e(this, "state", void 0), e(this, "tokenizer", void 0), e(this, "inlineQueue", void 0), this.tokens = [], this.tokens.links = Object.create(null), this.options = t || m, this.options.tokenizer = this.options.tokenizer || new z(), this.tokenizer = this.options.tokenizer, this.tokenizer.options = this.options, this.tokenizer.lexer = this, this.inlineQueue = [], this.state = {
			inLink: !1,
			inRawBlock: !1,
			top: !0
		};
		...
		this.options.pedantic ? (n.block = P.pedantic, n.inline = F.pedantic) : this.options.gfm && (n.block = P.gfm, this.options.breaks ? n.inline = F.breaks : n.inline = F.gfm), this.tokenizer.rules = n;
	}
	static get rules() {
		return {
			block: P,
	...
	constructor(t) {
		e(this, "options", void 0), e(this, "parser", void 0), this.options = t || m;
	}
	space(e) {
		return "";
	}
	code({ text: e, lang: t, escaped: n }) {
		var r;
		let i = (r = (t || "").match(v.notSpaceStart)) == null ? void 0 : r[0], a = e.replace(v.endingNewline, "") + "\n";
...
`;
	...
	constructor(t) {
		e(this, "options", void 0), e(this, "renderer", void 0), e(this, "textRenderer", void 0), this.options = t || m, this.options.renderer = this.options.renderer || new V(), this.renderer = this.options.renderer, this.renderer.options = this.options, this.renderer.parser = this, this.textRenderer = new H();
	}
	static parse(e, n) {
	...
	constructor(t) {
		e(this, "options", void 0), e(this, "block", void 0), this.options = t || m;
	}
	preprocess(e) {
		return e;
	}
...
}, e(f, "passThroughHooks", /* @__PURE__ */ new Set([
	"preprocess",
	"postprocess",
	"processAllTokens",
	"emStrongMask"
])), e(f, "passThroughHooksRespectAsync", /* @__PURE__ */ new Set([
	"preprocess",
	"postprocess",
	"processAllTokens"
])), f), G = new class {
	constructor(...t) {
		e(this, "defaults", p()), e(this, "options", this.setOptions), e(this, "parse", this.parseMarkdown(!0)), e(this, "parseInline", this.parseMarkdown(!1)), e(this, "Parser", U), e(this, "Renderer", V), e(this, "TextRenderer", H), e(this, "Lexer", B), e(this, "Tokenizer", z), e(this, "Hooks", W), this.use(...t);
	}
	walkTokens(e, t) {
		let n = [];
...
K.options = K.setOptions = function(e) {
	return G.setOptions(e), K.defaults = G.defaults, h(K.defaults), K;
}, K.getDefaults = p, K.defaults = m, K.use = function(...e) {
	return G.use(...e), K.defaults = G.defaults, h(K.defaults), K;
...
function Je(e, { markdownAutoWrap: t }) {
	return qe(e.replace(/<br\/>/g, "\n").replace(/\n{2,}/g, "\n"));
}
t(Je, "preprocessMarkdown");
function Ye(e) {
	return e.split(/\\n|\n|<br\s*\/?>/gi).map((e) => {
		var t, n;
...
t(Ye, "nonMarkdownToLines");
function Xe(e, n = {}) {
	let r = Je(e, n), i = K.lexer(r), a = [[]], o = 0;
...
t(Xe, "markdownToLines");
function Ze(e) {
	return e ? `<p>${e.replace(/\\n|\n/g, "<br />")}</p>` : "";
}
t(Ze, "nonMarkdownToHTML");
function Qe(e, { markdownAutoWrap: r } = {}) {
	let i = K.lexer(e);
	function a(e) {
		if (e.type === "text") return r === !1 ? e.text.replace(/\n */g, "<br/>").replace(/ /g, "&nbsp;") : e.text.replace(/\n */g, "<br/>");
		if (e.type === "strong") {
			var t;
			return `<strong>${(t = e.tokens) == null ? void 0 : t.map(a).join("")}</strong>`;
		} else if (e.type === "em") {
			var i;
		...
		return n.warn(`Unsupported markdown: ${e.type}`), e.raw;
	}
...
t($e, "splitTextToChars");
function et(e, t) {
	return q(e, [], $e(t.content), t.type);
}
...
t(Y, "applyStyle");
var nt = 16384;
async function rt(e, t, n, r, l = !1, u = o()) {
	let d = e.append("foreignObject");
	d.attr("width", `${Math.min(10 * n, nt)}px`), d.attr("height", `${Math.min(10 * n, nt)}px`);
	...
	h.html(p), Y(h, t.labelStyle), h.attr("class", `${m} ${r}`), Y(f, t.labelStyle), f.style("display", "table-cell"), f.style("white-space", "nowrap"), f.style("line-height", "1.5"), n !== Infinity && (f.style("max-width", n + "px"), f.style("text-align", "center")), f.attr("xmlns", "http://www.w3.org/1999/xhtml"), l && f.attr("class", "labelBkg");
...
t(it, "computeWidthOfText");
function at(e, t, n) {
	var r;
	let i = e.append("text"), a = X(i, 1, t);
	Q(a, [{
		content: n,
		type: "normal"
	}]);
...
function ot(e, n, r, i = !1, a = !1) {
	...
	let u = 0;
	for (let n of r) {
		let r = /* @__PURE__ */ t((t) => it(s, o, t) <= e, "checkWidth"), i = r(n) ? [n] : tt(n, r);
...
async function $(e, t = {}) {
	...
	e.replace(/(fa[bklrs]?):fa-([\w-]+)/g, (e, r, i) => (n.push((async () => {
		let n = `${r}:${i}`;
		return await u(n) ? await d(n, void 0, { class: "label-icon" }) : `<i class='${c(e, t).replace(":", " ")}'></i>`;
	})()), e));
...
var st = /* @__PURE__ */ t(async (e, t = "", { style: a = "", isTitle: o = !1, classes: s = "", useHtmlLabels: c = !0, markdown: u = !0, isNode: d = !0, width: f = 200, addSvgBackground: p = !1 } = {}, m) => {
	if (n.debug("XYZ createText", t, a, o, s, c, d, "addSvgBackground: ", p), c) {
		let n = await $(l(u ? Qe(t, m) : Ze(t)), m), r = t.replace(/\\\\/g, "\\");
		return await rt(e, {
			isNode: d,
			label: i(t) ? r : n,
		...
		let n = l(t.replace(/<br\s*\/?>/g, "<br/>")), i = ot(f, e, u ? Xe(n.replace("<br>", "<br/>"), m) : Ye(n), t ? p : !1, !d);
		if (d) {
...
//#endregion
export { st as n, qe as r, at as t };
