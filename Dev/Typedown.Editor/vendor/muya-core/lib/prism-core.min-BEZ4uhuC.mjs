import { t as e } from "./rolldown-runtime-CWhphoD1.mjs";
	...
	var n = function(e) {
		var t = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i, n = 0, r = {}, i = {
			...
			highlightElement: function(t, n, r) {
				...
				if (i.hooks.run("before-sanity-check", c), (s = c.element.parentElement) && s.nodeName.toLowerCase() === "pre" && !s.hasAttribute("tabindex") && s.setAttribute("tabindex", "0"), !c.code) return i.hooks.run("complete", c), void (r && r.call(c.element));
				if (i.hooks.run("before-highlight", c), c.grammar) if (n && e.Worker) {
				...
				for (var m = 0; m < p.length; ++m) {
					...
					for (var x = h.pattern || h, S = r.next, C = c; S !== t.tail && !(d && C >= d.reach); C += S.value.length, S = S.next) {
						...
						if (!(w instanceof a)) {
							var T, E = 1;
							if (v) {
								if (!(T = o(x, C, e, _)) || T.index >= e.length) break;
								var D = T.index, O = T.index + T[0].length, k = C;
								for (k += S.value.length; D >= k;) k += (S = S.next).value.length;
								if (C = k -= S.value.length, S.value instanceof a) continue;
								for (var A = S; A !== t.tail && (k < O || typeof A.value == "string"); A = A.next) E++, k += A.value.length;
								E--, w = e.slice(C, k), T.index -= C;
							} else if (!(T = o(x, 0, w, _))) continue;
							D = T.index;
							var j = T[0], M = w.slice(0, D), N = w.slice(D + j.length), P = C + w.length;
							d && P > d.reach && (d.reach = P);
							var F = S.prev;
							if (M && (F = l(t, F, M), C += M.length), u(t, F, E), S = l(t, F, new a(f, g ? i.tokenize(j, g) : j, y, j)), N && l(t, S, N), E > 1) {
								var I = {
									cause: f + "," + m,
									reach: P
								};
