...
import { $ as i, J as a, a as o, b as s, et as c, nt as l, o as u, x as d } from "./chunk-WYO6CB5R-_U4dPty4.mjs";
import { t as f } from "./arc-B3Og6pzD.mjs";
import { p } from "./chunk-ICXQ74PX-5xZ0PKaW.mjs";
...
e(K, "wrap");
var ie = /* @__PURE__ */ e(function(e, t, n, i, a, o = !1) {
	var s, c, l;
...
}, se = /* @__PURE__ */ e(function(e, t, i, o) {
	var s, c, l, u, f, p;
	...
	S === "sandbox" && (C = r("#i" + t));
	let w = r(S === "sandbox" ? C.nodes()[0].contentDocument.body : "body").select("#" + t);
	w.append("g");
	let T = o.db.getTasks(), E = o.db.getCommonDb().getDiagramTitle();
	n.debug("task", T), q.initGraphics(w, t);
	...
	D.forEach(function(e) {
		let t = {
			...
			padding: 20,
			maxHeight: O
		}, r = q.getVirtualNodeHeight(w, t, m);
		n.debug("sectionHeight before draw", r), O = Math.max(O, r + 20);
	});
	let I = 0, L = 0;
	n.debug("tasks.length", T.length);
	for (let [e, t] of T.entries()) {
		...
		n.debug("taskHeight before draw", i), k = Math.max(k, i + 20), I = Math.max(I, t.events.length);
		let a = 0;
		for (let e of t.events) {
			...
			a += q.getVirtualNodeHeight(w, n, m);
		}
		t.events.length > 0 && (a += (t.events.length - 1) * 10), L = Math.max(L, a);
	}
	n.debug("maxSectionHeight before draw", O), n.debug("maxTaskHeight before draw", k), D && D.length > 0 ? D.forEach((e) => {
		...
		let a = w.append("g"), o = q.drawNode(a, i, P, m, t);
		n.debug("sectionNode output", o), a.attr("transform", `translate(${M}, ${j})`), N += O + 50, r.length > 0 && J(w, r, P, M, N, k, m, I, L, O, !1, t), M += 200 * Math.max(r.length, 1), N = j, P++;
	}) : (F = !1, J(w, T, P, M, N, k, m, I, L, O, !0, t));
	let R = w.node().getBBox();
	if (n.debug("bounds", R), E && w.append("text").text(E).attr("x", h === "neo" ? R.x * 2 + x : R.width / 2 - x).attr("font-size", "4ex").attr("font-weight", "bold").attr("y", 20), A = F ? O + k + 150 : k + 100, w.append("g").attr("class", "lineWrapper").append("line").attr("x1", x).attr("y1", A).attr("x2", R.width + 3 * x).attr("y2", A).attr("stroke-width", 4).attr("stroke", "black").attr("marker-end", `url(#${t}-arrowhead)`), h === "neo" && v && g !== "neutral") {
	...
	a(void 0, w, (l = (u = m.timeline) == null ? void 0 : u.padding) == null ? 50 : l, (f = (p = m.timeline) == null ? void 0 : p.useMaxWidth) != null && f);
}, "draw"), J = /* @__PURE__ */ e(function(e, t, r, i, a, o, s, c, l, u, d, f) {
	for (let c of t) {
		var p;
		let t = {
			descr: c.task,
		...
		n.debug("taskNode", t);
		let u = e.append("g").attr("class", "taskWrapper"), m = q.drawNode(u, t, r, s, f).height;
		if (n.debug("taskHeight after draw", m), u.attr("transform", `translate(${i}, ${a})`), o = Math.max(o, m), c.events) {
	...
	a -= 10;
}, "drawTasks"), ce = /* @__PURE__ */ e(function(e, t, r, i, a, o, s) {
	let c = 0, l = a;
	a += 100;
	for (let l of t) {
		let t = {
		...
		n.debug("eventNode", t);
		let u = e.append("g").attr("class", "eventWrapper"), d = q.drawNode(u, t, r, o, s, !0).height;
	...
	draw: se
}, Y = 200, X = 5, ue = Y + X * 2, Z = Y + 100, de = Z + X * 2, Q = 10, fe = 0, pe = 20, me = 20, he = 30, ge = 50, _e = /* @__PURE__ */ e(function(e, t, r, i) {
	var o, s, c, l, u, f, h;
	...
	n.debug("task", y), q.initGraphics(v);
	...
	x.forEach(function(e) {
		let t = {
			...
			padding: X,
			maxHeight: S
		}, r = q.getVirtualNodeHeight(v, t, g);
		n.debug("sectionHeight before draw", r), S = Math.max(S, r);
	});
	let F = 0;
	...
	for (let [e, t] of y.entries()) {
		...
		n.debug("taskHeight before draw", i), C = Math.max(C, i);
		let a = 0;
		for (let e of t.events) {
			let n = {
				descr: e,
			...
			a += q.getVirtualNodeHeight(v, n, g);
		}
		t.events.length > 0 && (a += (t.events.length - 1) * Q), F = Math.max(F, a) + fe;
	}
	n.debug("maxSectionHeight before draw", S), n.debug("maxTaskHeight before draw", C);
	...
	M ? x.forEach((e) => {
		...
		let s = T + a.height + pe;
		t.length > 0 && $(v, t, j, N, s, C, g, I, !1);
		let c = t.length, l = a.height + pe + I * Math.max(c, 1) - (c > 0 ? he * 2 : 0);
		T += l, j++;
	}) : $(v, y, j, N, T, C, g, I, !0);
	let L = (c = v.node()) == null ? void 0 : c.getBBox();
	if (!L) throw Error("bbox not found");
	if (n.debug("bounds", L), b) {
		var R;
		...
		n.debug("bounds after title", L);
	}
	let [z] = p(g.fontSize), B = (z == null ? 16 : z) * 2, V = (z == null ? 16 : z) * .5 + 20, H = v.append("g").attr("class", "lineWrapper");
	H.append("line").attr("x1", N).attr("y1", E - B).attr("x2", N).attr("y2", L.y + L.height + V).attr("stroke-width", 4).attr("stroke", "black").attr("marker-end", "url(#arrowhead)"), H.lower(), a(void 0, v, (l = (u = g.timeline) == null ? void 0 : u.padding) == null ? 50 : l, (f = (h = g.timeline) == null ? void 0 : h.useMaxWidth) != null && f);
	...
	for (let d of t) {
		...
		n.debug("taskNode", t);
		let f = e.append("g").attr("class", "taskWrapper"), p = q.drawNode(f, t, r, s), m = p.height;
		n.debug("taskHeight after draw", m);
		let h = i - me - p.width;
	...
	for (let o of t) {
		...
		n.debug("eventNode", t);
		let l = e.append("g").attr("class", "eventWrapper"), u = q.drawNode(l, t, r, s).height;
