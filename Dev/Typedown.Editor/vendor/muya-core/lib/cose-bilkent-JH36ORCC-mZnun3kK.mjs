...
import { t as a } from "./cytoscape.esm-DJ_KYwtv.mjs";
...
var g = /* @__PURE__ */ n(async (e, t, { insertCluster: n, insertEdge: r, insertEdgeLabel: i, insertMarkers: a, insertNode: o, log: s, positionEdgeLabel: c }, { algorithm: l }) => {
	...
	s.debug("Positioning nodes based on layout results"), v.nodes.forEach((e) => {
		let t = u[e.id];
		t != null && t.domId && (t.domId.attr("transform", `translate(${e.x}, ${e.y})`), t.x = e.x, t.y = e.y, s.debug(`Positioned node ${t.id} at center (${e.x}, ${e.y})`));
	}), v.edges.forEach((t) => {
		...
		var n, a;
		await i(g, t);
		let o = u[(n = t.start) == null ? "" : n], l = u[(a = t.end) == null ? "" : a];
		if (o && l) {
			let n = v.edges.find((e) => e.id === t.id);
			if (n) {
				s.debug("APA01 positionedEdge", n);
				let i = { ...t };
				c(i, r(h, i, d, e.type, o, l, e.diagramId));
			} else {
				let n = {
					...t,
					points: [{
				...
				c(n, r(h, n, d, e.type, o, l, e.diagramId));
			}
		}
