...
[DoNotNotify]
        public sealed partial class ImageSelector : UserControl
    {
        ...
        private readonly Flyout flyout = new() { Placement = PlacementMode.Bottom };
        ...
        private void OnFlyoutClosing(object? sender, System.ComponentModel.CancelEventArgs args)
        {
        ...
        private async void OnSaveButtonClick(object sender, RoutedEventArgs e)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (topLevel != null)
                {
                    await AppContentDialog.Create(Locale.GetString("Error"), ex.Message, Locale.GetDialogString("Ok")).ShowAsync(topLevel);
                }
            }
            finally
            {
