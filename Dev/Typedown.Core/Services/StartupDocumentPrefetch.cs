...
namespace Typedown.Core.Services
{
    ...
    /// <summary>
    /// 启动文档在磁盘上的快照：文件是否存在、解码后的正文与格式、备份。只含文件 IO 的结果，不改任何 ViewModel 状态、不弹对话框；
    /// 读正文失败时保存原异常，由装载方在 UI 线程上按原来的路径重新抛出。
    /// </summary>
    public sealed class StartupFileSnapshot
    {
        private readonly DecodedTextFile? file;
        private readonly ExceptionDispatchInfo? readError;

        private StartupFileSnapshot(string path, bool exists, DecodedTextFile? file, ExceptionDispatchInfo? readError, string? backup)
        {
            Path = path;
            Exists = exists;
            this.file = file;
        ...
        /// <summary>按 <see cref="TextFileCodec"/> 解码的正文（换行已统一成 <c>\n</c>）与原格式；读取失败时重新抛出当时的异常。</summary>
        public DecodedTextFile GetFile()
        {
            readError?.Throw();
            return file ?? throw new FileNotFoundException("File does not exist.", Path);
        }

        /// <summary>正文；读取失败时重新抛出当时的异常。</summary>
        public string GetText() => GetFile().Text;
        ...
        public static async Task<StartupFileSnapshot> ReadAsync(string path, AutoBackup autoBackup)
        {
            ...
            DecodedTextFile file;
            try
            {
                file = await TextFileCodec.ReadAsync(path).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
            ...
            return new(path, true, file, null, backup);
        }
    }
