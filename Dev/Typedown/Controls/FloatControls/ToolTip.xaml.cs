...
namespace Typedown.Controls.FloatControls
{
    public sealed partial class ToolTip : UserControl
    {
        ...
        public async void Open(string text)
        {
            ...
            var flyoutPosition = new Point(pos.X, pos.Y - 16);
            ContentTextBlock.Text = text;
            flyout.AreOpenCloseAnimationsEnabled = false;
            flyout.OverlayInputPassThroughElement = ((XamlRoot)ViewModel.ViewRoot).Content;
            flyout.FlyoutPresenterStyle = Resources["ToolTipFlyoutStyle"] as Style;
            flyout.Content = this;
            flyout.ShowAt(editor, new() { Placement = FlyoutPlacementMode.Top, ShowMode = FlyoutShowMode.TransientWithDismissOnPointerMoveAway, Position = flyoutPosition });
            hideTimer.Start();
        }
