...
namespace Typedown.Controls.EditorControls.ContextMenuItems
{
    public sealed partial class ImageItem : MenuItemCollection, INotifyPropertyChanged
    {
        ...
        private async void OnUploadImageClick(ImageUploadConfig config)
        {
            ...
            {
                await AppContentDialog.Create(Locale.GetDialogString("UploadFailedTitle"), ex.Message, "Ok").ShowAsync((XamlRoot)ViewModel.ViewRoot);
            }
        }
        ...
        {
            ViewModel.ServiceProvider.GetRequiredService<IMarkdownEditor>().PostMessage("ReplaceImage", new
            {
                htmlImgTag.Src,
                htmlImgTag.Alt,
                htmlImgTag.Title,
