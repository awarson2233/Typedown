...
- **本地图片。** 页面不能读 `file://`，本地图片一律改写成宿主拦截的专用主机：`https://typedown.image/<盘符>/<路径段>`，每个路径段单独 `encodeURIComponent`，例如 `C:\Users\me\图 1.png` 写成 `https://typedown.image/C/Users/me/%E5%9B%BE%201.png`。改写在页面内完成（[imageUrl.ts](/Dev/Typedown.Editor/src/renderers/imageUrl.ts)，Markdown 图片与 HTML 块里的 `<img>` 共用）：相对路径按 `doc.load` 的 `basePath` 解析，`/a.png` 指 `basePath` 所在盘的根目录，`file:///C:/…` 与 `\\?\C:\…` 转成同样的绝对路径，本地路径里的百分号转义先解码，`.`、`..` 在页面内消解（越过盘符根目录的不加载）；`http(s):`、`data:image/`、`blob:` 原样使用；UNC 路径（`\\server\share`、`//server/share`、`file://server/…`）与其他协议不加载，直接显示失败占位。宿主（[WinUIEditorHost](/Dev/Typedown.WinUI/Controls/EditorControls/Hosting/WinUIEditorHost.cs)）用 `AddWebResourceRequestedFilter("https://typedown.image/*", Image)` 只拦图片上下文的请求（fetch、XHR 拿不到本地文件），路径校验在 Core 的 [LocalImageRequest](/Dev/Typedown.Core/Editor/Wire/LocalImageRequest.cs)：只接受 GET / HEAD（否则 405）；第一段必须是单个盘符字母，其余各段解码后不得为空、`.`、`..`，不得含 `\ / : * ? " < > |` 与控制字符，不得以点或空格结尾，不得是设备名（`NUL`、`COM1.png` 之类），扩展名只能是 png、apng、jpg、jpeg、jfif、gif、webp、avif、bmp、ico、svg，拼出的路径经 `Path.GetFullPath` 规范化后必须与原样相同，任一不满足回 403；文件不存在或读不了回 404，超过 64 MB 回 403。成功的响应带对应的 `Content-Type`、`Cache-Control: no-cache`、`X-Content-Type-Options: nosniff` 与 `Content-Security-Policy: default-src 'none'; style-src 'unsafe-inline'`（SVG 里的脚本与外链不执行）。
