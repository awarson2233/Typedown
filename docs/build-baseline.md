...
- Editor static bundle owner: `Dev\Typedown.Editor` owns editor source and generated bundle content. `Dev\Typedown.WinUI\Resources\Statics` is the active staging path consumed by WinUI. `Typedown.WinUI` references `Typedown.Editor`, so normal Visual Studio/MSBuild builds run `yarn build` first and refresh the generated Content items before output copy and packaging.
